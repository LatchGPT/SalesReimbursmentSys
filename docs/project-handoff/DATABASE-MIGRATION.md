...
- `backend/prisma/schema.prisma` was produced by introspecting production, not by transcribing the retired application schema.
- All five repository modules under `backend/src/db/` use the singleton from `backend/src/db/index.ts`.
- The 25 existing application tables, 16 enums, and `claim_number_seq` are represented in `backend/prisma/migrations/20260918000000_baseline/migration.sql`.
...
`DIRECT_URL` is the administrative connection used by `backend/prisma.config.ts` for introspection and Prisma Migrate. Do not use it in browser code or routine application traffic.
